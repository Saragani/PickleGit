# PickleGit → GitKraken-Class Git Client — Master Plan

## Context

PickleGit (WPF, .NET 4.7.2, LibGit2Sharp 0.27.2, MVVM) has strong foundations: custom commit graph with Bézier lanes and ref pills, animated drag-reorder tabs, dark theme, staging panel, clone/fetch/pull/push with a smart credential chain, per-repo startup cache. But a full audit shows it is missing the majority of what makes GitKraken a daily driver — both git capabilities (no rebase, revert, reset, amend, conflict resolution, hunk staging, stash apply/drop…) and UX surfaces (zero keyboard shortcuts, almost no context menus, no search, no settings window, VB InputBox prompts).
...
- No drag-and-drop branch operations (GitKraken's signature interaction).
- No PR/hosting awareness, no avatars (initials only), no submodule/worktree/reflog panels.
- No undo (GitKraken's trust feature), no commit-and-push, no amend checkbox, no 50/72 message guides.
...
### Phase 4 — Rebase + drag-drop (GitKraken signature features)
...
1. `msbuild MyGitClient.sln` (or Rebuild in VS) — zero warnings introduced.
